...
class Maliciousclient(Client):
    ...
    def train(self):
        ...
        ## 第一个客户端为攻击方
        if self.client_id == 0:
            label_a = torch.randint(1, 9, (1, 1))
            gen_image, gen_label=GAN.GAN_attack(self.client_model, num_classes=1, batch=1, img_size=self.img_size, attack_label=label_a, dataset=self.train_loader.dataset)
